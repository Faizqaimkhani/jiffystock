...
// on close chat close the chat box but don't remove it from the dom4
 ...
 */
function loaderHtml() {
    return '<i class="glyphicon glyphicon-refresh loaderchat"></i>';
}
...
function loadLatestMessages(container, user_id) {
    ...
    $.ajax({
        url: base_url + "/load-latest-messages-user",
...
function fetchOldMessages(to_user, old_message_id) {
    ...
    $.ajax({
        url: base_url + "/fetch-old-messages-user",
