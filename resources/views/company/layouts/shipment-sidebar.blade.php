@section('nav_identification')
    @if(Auth::guard('shipment_user')->user()->badge_verification_status == 2)
     <p class="dropdown-item ">{{Auth::guard('shipment_user')->user()->name}} <i class="fa fa-check" style="font-size: 14px;color: #7599e9;s" aria-hidden="true"></i></p>
     @else
     <p class="dropdown-item ">{{Auth::guard('shipment_user')->user()->name}}</p>
     @endif
     <small class="dropdown-item">{{Auth::guard('shipment_user')->user()->email}} </small>
@endsection
  ...
  <div class="sidebar-wrapper" id="sidebar-wrapper">
    <ul class="nav">
      <li class="@if ($activePage == 'home') active @endif">
        <a href="{{ route('shipment.index') }}" aria-expanded="false">
      ...
      <li class="@if ($activePage == 'orders') active @endif">
        <a href="{{route('shipment.orders')}}" aria-expanded="false"><i class="now-ui-icons design_app"></i><p>Orders</p></a> </li>
        <li class="@if ($activePage == 'send_email') active @endif"><a href="{{ route('shipment.send.email') }}" aria-expanded="false"><i class="now-ui-icons design_app"></i><p>Send Email</p></a></li>
