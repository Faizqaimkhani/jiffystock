...
<div class="site__body">
    ...
    <div class="page-header">
        <div class="page-header__container container">
            ...
            @if(count($products) > 0)
            <div class="bg-brand px-3 py-2">
              <h5 class="text-white mt-2">Sort By</h5>
              <form action="{{ route('search') }}" method="get">
                  <div class="form-group">
                    <div class="row">
                        <div class="col">
                            <select name="country" class="form-control" id="country" value="{{ request('country') }}">
                        ...
                        <div class="col">
                          <input type="text" placeholder="Min Price" name="min_price" class="form-control" value="{{ request('min_price') }}">
                        </div>
                        <div class="col">
                          <input type="text" placeholder="Max Price" name="max_price" class="form-control" value="{{ request('max_price') }}">
                        </div>
                        <div class="col">
                            <select name="category" class="form-control" id="category" value="{{ request('category') }}">
                        ...
                        <div class="col">
                            <select name="supplier" class="form-control" id="supplier" value="{{ request('supplier') }}">
                                   ...
                                   <div class="products-list__item">
                                      <div class="product-card"><button class="product-card__quickview" type="button"><svg width="16px" height="16px">
                                          ...
                                          <div class="product-card__info">
                                              <div class="product-card__name"><a href="/product-details/{{$p->id}}">{{$p->name}}
                                                  </a></div>
                                              <div class="d-flex">
                                                  <div class="flag_custom">
                                                      {{ flag(getCountryCode($p->user->countries->name)) }}
                                                  </div>
                                                  <small class="ml-2 mt-1"> {{$p->user->countries->name}}</small>
                                              </div>
                                              <div class="d-flex">
                                                  <small>Stock : </small><small class="ml-2">{{$p->stock_quantity}}</small>
                                          ...
                                          <div class="product-card__actions">
                                              ...
                                              <div class="product-card__prices">{{currentCurrency($p->price)}} / {{$p->unit}}</div>
                                              <div class="product-card__buttons">
                                                  ...
                                                  <a href="{{ url('add-to-cart/'.$p->id) }}" onclick=" return confirm('If Product is From Another Supplier Your Previous Cart Products will lossed?')" class="btn btn-secondary product-card__addtocart product-card__addtocart--list" role="button">Add To Cart</a>
                                                   <button class="btn btn-light btn-svg-icon btn-svg-icon--fake-svg product-card__wishlist" onclick="add_to_wishlist({{$p->id}})" type="button"><svg width="16px" height="16px">
